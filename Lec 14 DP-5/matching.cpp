...
int f(int currMan,int vis){
    // base case
    if(currMan==n+1) return 1;

    // calculate se phle check
    if(dp[currMan][vis]!=-1) return dp[currMan][vis];
    ...
    for(int woman = 1 ; woman<=n ; woman++){
        ...
        // op2
        ans = (ans+f(currMan+1,vis|mask)) % MOD;  // by value
    }
    return dp[currMan][vis] = ans;
}
